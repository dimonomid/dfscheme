    ...
    if (isNull(expr) || isJSFunc(expr) || isLambda(expr)) {
      ...
      if (expr[0] === "quote") {
        ...
        if (isPair(expr[1])) {
          ...
          setToScope(scope, expr[1], evalInScope(expr[2], scope));
        }
        ...
        do {
          ...
          try {
            if (isJSFunc(f)) {
              ...
              var needArgsCnt = isPair(f.expr[1]) ? f.expr[1].length : 0;
