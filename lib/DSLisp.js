...
function DSLisp() {
  ...
  function qq(expr, scope) {
    ...
    if (isPair(expr)) {
      if (expr.car === "unquote") {
        ...
        ret = new Pair({
          car: qq(expr.car, scope),
          cdr: qq(expr.cdr, scope)
        });
      }
    } else {
      ret = expr;
  ...
  function evalInScope(expr, scope) {
    ...
    if (isNull(expr) || isJSFunc(expr) || isLambda(expr)) {
      ...
      if (expr.car === "quote") {
        ...
        var val = boolValue(evalInScope(expr.car, scope));
        if (val !== false) {
          expr = expr.cdr;
          curCSItem().tail = true;
        ...
        if (isPair(expr.car)) {
          throw new Error("TODO: lambda syntax sugar");
        ...
        if (!isNull(expr.car) && !isPair(expr.car)) {
          throw new Error("Ill-formed special form lambda: second item should be a list, " + stringify(expr.car) + " given");
        }
        ...
        ret = {
          args: rootExpr.cdr.car,
          exprs: rootExpr.cdr.cdr,
          lbd: true,
          scope: scope,
        };
          ...
          try {
            if (isJSFunc(f)) {
              ...
              var newScope = augmentScope(f.scope);
                ...
                /*
              var needArgsCnt = isPair(f.args) ? f.args.length : 0;
              ...
              for (i = 0; i < needArgsCnt; i++) {
                setToScope(newScope, f.expr[1][i], args[i]);
              }
              */
  ...
  function exec(str) {
    return stringify(evaluateEach(parse(tokenize(str))));
  }
